...
vida curta e um token de renovação de vida longa.
...
- **THEN** a resposta é `200` com o token de acesso, o token de renovação e os
  dados do usuário
...
O sistema MUST permitir trocar um token de renovação válido por um novo par de
tokens, sem exigir as credenciais novamente.

Cada renovação MUST invalidar o token apresentado e emitir um novo — o token
de renovação é de uso único.
...
- **WHEN** um token de renovação válido e não expirado é apresentado
- **THEN** a resposta é `200` com um novo par de tokens, e o novo token de
  renovação difere do anterior
...
O sistema MUST permitir revogar um token de renovação, encerrando aquela
sessão.

A operação MUST ser idempotente e MUST NOT revelar se o token existia.

#### Scenario: Logout

- **WHEN** um token de renovação é enviado para encerramento
- **THEN** a resposta é `204` e o token deixa de servir para renovação

#### Scenario: Logout repetido

- **WHEN** o mesmo token é enviado para encerramento novamente
