...
- [x] 1.1 Adicionar `mobileRefreshSchema` (`{ body: { refreshToken:
...
- [x] 2.1 Adicionar `mobileLogin`, `mobileRefresh`, `mobileLogout` a
...
- [x] 2.2 Registrar `POST /auth/mobile/login` (com `loginSchema`),
...
- [x] 3.1 Com `curl`: `POST /auth/mobile/login` com credenciais corretas
      responde `200` com `refreshToken` no corpo e sem `Set-Cookie` no
      cabeçalho da resposta (`curl -i` e inspecionar os headers)
- [x] 3.2 `POST /auth/mobile/refresh` com o `refreshToken` da resposta
      anterior responde `200` com um `refreshToken` novo e diferente,
      ainda sem `Set-Cookie`
- [x] 3.3 Reapresentar o `refreshToken` já trocado no passo 3.2 responde
      ...
      web
- [x] 3.4 `POST /auth/mobile/logout` com um `refreshToken` válido responde
      `204`, e uma tentativa de `POST /auth/mobile/refresh` com o mesmo
      token depois do logout responde `401`
- [x] 3.5 `POST /auth/mobile/refresh` sem `refreshToken` no corpo responde
      `400`; `POST /auth/mobile/logout` sem `refreshToken` no corpo
      responde `204` sem tentar revogar nada
- [x] 3.6 Repetir o roteiro de `curl` já existente para `/auth/login`,
...
- [x] 4.1 Atualizar `README.md` com as três rotas novas, seguindo o estilo
      da seção de autenticação já existente
- [x] 4.2 `npm run typecheck` passa sem erros
- [x] 4.3 `openspec validate --specs` passa para `user-auth`
