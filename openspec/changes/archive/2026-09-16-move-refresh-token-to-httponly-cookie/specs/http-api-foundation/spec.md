...
Credenciais (cookies) MUST só ser aceitas entre origens quando a lista de
origens permitidas é específica. Com qualquer origem liberada, credenciais
MUST NOT ser aceitas — a combinação é recusada pelo próprio navegador, e
aceitá-la do mesmo jeito exporia cookies de sessão a qualquer site que fizesse
a requisição.
...
#### Scenario: Credenciais com lista de origens específica

- **WHEN** a lista de origens permitidas é específica e a requisição vem de
  uma origem permitida
- **THEN** a resposta inclui `Access-Control-Allow-Credentials: true`

#### Scenario: Credenciais com qualquer origem liberada

- **WHEN** qualquer origem está liberada (`CORS_ORIGIN` vazio ou `*`)
- **THEN** a resposta NÃO inclui `Access-Control-Allow-Credentials: true`
